...
import sn.fatou.dakpark.config.KafkaProperties;

class ReservationDeParkingKafkaResourceIT {
    ...
    void setup() {
        ...
        ReservationDeParkingKafkaResource kafkaResource = new ReservationDeParkingKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/reservation-de-parking-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/reservation-de-parking-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
